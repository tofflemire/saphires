...
def bf_read_target_fits(spectra_list,w_mult=1.0,combine_all=True,norm=False,
	...
	if combine_all == True:
		...
		for i in range(t_f_names.size):
			w_all = np.append(w_all,t_spectra[t_f_names[i]][1])
			flux_all = np.append(flux_all,t_spectra[t_f_names[i]][0])
			
			if i == 0:
				w_range_all = t_spectra[t_f_names[i]][8]+','
			if ((i > 0) & (i<t_f_names.size-1)):
				w_range_all = w_range_all+t_spectra[t_f_names[i]][8]+','
			if i == t_f_names.size-1:
				w_range_all = w_range_all+t_spectra[t_f_names[i]][8]
